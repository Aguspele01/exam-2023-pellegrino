...
const message = `Hay 4 personas en el developer team. Sus nombres son:\n${people.map(person => ${person.name}\n)
console.log('Ejercico _1  ->', message)

const team = [
  { name: "Jennie", senior: true },
  { name: "Ronald", senior: false },
  { name: "Martin", senior: true },
  { name: "Anneli", senior: false }
];

const message = `Hay ${team.length} personas en el developer team.\nSus nombres son ${team.map(person => person.name).join(", ")}. \n${team.filter(person => person.senior).length} de ellos son senior con experiencia.`;
...
const message = `Hay ${team.length} personas en el developer team.\nSus nombres son:\n${team.map(person => `${person.name}\n`).join("")}${team.filter(person => person.senior).length} de ellos son senior con experiencia.`;

console.log(message);

const team = [
  { name: "Jennie", senior: true, role: "Desarrollador" },
  { name: "Ronald", senior: false, role: "Diseñador" },
  { name: "Martin", senior: true, role: "Desarrollador" },
  { name: "Anneli", senior: false, role: "Tester" }
];

const message = `Hay ${team.length} personas en el developer team.\nSus nombres y roles son:\n${team.map(person => `${person.name} - ${person.role}\n`).join("")}${team.filter(person => person.senior).length} de ellos son senior con experiencia.`;
...
const resultados = operaciones.map(({ num1, num2 }) => num1 * num2);
console.log('Ejercicio _3_c ->', resultados)
...
const final = semiFinal[Math.floor(Math.random() * semiFinal.length)];
const finalResultado = `¡${final.pais} es el campeón!`;

const campeon = final.pais;
...
const final = semiFinal.filter(objeto => objeto.pais === 'Argentina' || objeto.pais === 'Brasil');

const campeon = final.length === 1 ? final[0].pais : '';
...
const finalResultado = final.map(objeto => ({ ...objeto, goles: Math.floor(Math.random() * 101) }));

console.log('Ejercicio _4 ->', finalResultado);
...
const campeon = finalResultado[0].goles > finalResultado[1].goles
  ? `El campeón es ${finalResultado[0].pais}.`
  : `El campeón es ${finalResultado[1].pais}.`;
